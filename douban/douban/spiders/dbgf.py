...
class DbgfSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for i in range(0,5):
            yield scrapy.Request(url='https://movie.douban.com/j/search_subjects?type=movie&tag=%E8%B1%86%E7%93%A3%E9%AB%98%E5%88%86&sort=recommend&page_limit=20&page_start='+str(i*20),
            headers=self.headers,callback=self.next_req)

    def next_req(self,response):
        data = json.loads(response.body)
        subjects = data['subjects']
        for item in subjects:
            yield scrapy.Request(url=item['url'],headers=self.headers,callback=self.parse)

    def parse(self, response):
        obj = DoubanItem()
        info = response.xpath('//div[@id="info"]').extract_first()
        infoSelector = Selector(text = info)
        obj['movieName'] = response.xpath('//span[@property="v:itemreviewed"]/text()').extract_first()
        obj['director'] = infoSelector.xpath('//a[@rel="v:directedBy"]/text()').extract_first()
        ...
        obj['initialReleaseDate'] = infoSelector.xpath('//span[@property="v:initialReleaseDate"]/text()').extract_first()
        obj['runtime'] = infoSelector.xpath('//span[@property="v:runtime"]/text()').extract_first()
        # 转换成字符串用来正则匹配
